...
export async function removeFolderFiles(supabase, folder) {
  await removeFolderFilesFromBucket(supabase, 'tracks', folder)
}

// Sletter alle billeder i en brugers mappe i bucketten "images"
export async function removeFolderImages(supabase, folder) {
  await removeFolderFilesFromBucket(supabase, 'images', folder)
}

async function removeFolderFilesFromBucket(supabase, bucket, folder) {
  for (let i = 0; i < 50; i++) {
    ...
    if (!removed || removed.length === 0) {
      throw new Error('Kunne ikke slette alle filerne. Prøv igen.')
    }
  }
}

export function imageTooBigMessage(bytes) {
  const mb = (bytes / 1024 / 1024).toFixed(1)
  return `Billedet er ${mb} MB, og grænsen er ${MAX_IMAGE_MB} MB. Vælg et mindre billede.`
}
...
export async function uploadImage(supabase, userId, kind, file) {
  if (!file.type.startsWith('image/')) {
    throw new Error('Filen skal være et billede (JPG, PNG eller WEBP).')
  }
  if (file.size > MAX_IMAGE_BYTES) {
    throw new Error(imageTooBigMessage(file.size))
  }
